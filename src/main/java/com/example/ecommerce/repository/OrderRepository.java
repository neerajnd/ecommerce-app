...
public interface OrderRepository extends JpaRepository<Order, Long>{
	...
	@Query(value = "select sum(po.product_price) from orders o inner join product_order po on o.id = po.order_id inner join product p on po.product_id = p.id where (date(o.created_at) between :startDate and :endDate) and o.user_id = :userId", nativeQuery = true)
	Float getTotalPriceOfOrdersBetweenDates(Long userId, Date startDate, Date endDate);
}
